...
export default function ModalWrapper({ children, title, className = '' }: PropsWithChildren<Props>) {
  const closeModal = useToggleModal();
  return (
    <div className="w-full h-screen z-40 bg-modalBlur block top-0 fixed">
      <div className="h-full w-full flex items-center justify-center">
        <div className={`w-full h-screen sm:w-96 sm:h-96 md:h-[480px] md:w-[480px] bg-white flex flex-col rounded shadow ${className}`}>
