...
const createUser = async (req, res, next) => {
    ...
    if (!isValid) {
        res.status(400).send({ error: { message: 'Failed to create user. Invalid request body.', errors }})
        return;
    }
    ...
    User.findOne({ email: req.body.email }).then((user) => {
        if (user) {
            ...
            newUser.save()
                .then((user) => {
                    res.user = { username: user.username, id: user._id };
                    res.message = { message: 'Successfully created user.', user: pick(['id', 'username', 'email', 'fname', 'lname', 'created_at'], user) };
                    next();
                })
                .catch((error) => {
